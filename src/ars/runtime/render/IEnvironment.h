...
class IEnvironment {
  public:
    ...
    // alloc or set.
    virtual std::shared_ptr<ITexture> irradiance_cube_map() = 0;
    // Set a precomputed cube map for IBL.
    // When call update_irradiance_cube_map, the content of the cube map will be
    // modified.
    virtual void
    set_irradiance_cube_map(const std::shared_ptr<ITexture> &cube_map) = 0;
    // Alloc a new cube map with given size for IBL. One should call
    // update_irradiance_cube_map() to update its content.
    virtual void alloc_irradiance_cube_map(uint32_t resolution) = 0;
    ...
    virtual void update_irradiance_cube_map() = 0;
};
} // namespace ars::render