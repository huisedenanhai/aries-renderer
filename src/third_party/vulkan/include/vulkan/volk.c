...
typedef PFN_vkVoidFunction (*vkGetDeviceProcAddrStub)(void* context, const char* name);
...
{
  volkLoadDeviceCustom(device, vkGetDeviceProcAddr);
}
#endif

void volkLoadDeviceCustom(VkDevice device, PFN_vkGetDeviceProcAddr handle) {
  loadedDevice = device;
  volkGenLoadDevice(device, (vkGetDeviceProcAddrStub)(handle));
}
...
{
  volkLoadDeviceTableCustom(table, device, vkGetDeviceProcAddr);
}
#endif

void volkLoadDeviceTableCustom(struct VolkDeviceTable* table, VkDevice device, PFN_vkGetDeviceProcAddr handler) {
  volkGenLoadDeviceTable(table, device, (vkGetDeviceProcAddrStub)(handler));
}
...
#ifndef VOLK_NO_INSTANCE_PROTOTYPES
/* VOLK_GENERATE_INSTANCE_PROTOTYPES_C */
#if defined(VK_VERSION_1_0)
