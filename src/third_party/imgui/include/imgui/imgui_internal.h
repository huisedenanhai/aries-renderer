...
// [SECTION] ImGui (main imgui context)
...
// A better solution may involve appending into a log of colors in ImGui + store offsets into those arrays in ImGuiWindow,
...
{
    bool                    Initialized;
    bool                    FontAtlasOwnedByContext;            // IO.Fonts-> is owned by the ImGui and will be destructed along with it.
    ImGuiIO                 IO;
    ...
    // Extensions
    // FIXME: We could provide an API to register one slot in an array held in ImGui?
...
#ifdef IMGUI_ENABLE_TEST_ENGINE
extern void         ImGuiTestEngineHook_ItemAdd(ImGui* ctx, const ImRect& bb, ImGuiID id);
extern void         ImGuiTestEngineHook_ItemInfo(ImGui* ctx, ImGuiID id, const char* label, ImGuiItemStatusFlags flags);
extern void         ImGuiTestEngineHook_IdInfo(ImGui* ctx, ImGuiDataType data_type, ImGuiID id, const void* data_id);
extern void         ImGuiTestEngineHook_IdInfo(ImGui* ctx, ImGuiDataType data_type, ImGuiID id, const void* data_id, const void* data_id_end);
extern void         ImGuiTestEngineHook_Log(ImGui* ctx, const char* fmt, ...);
